...
class ServicioSincronizacion {
  ...
  final ServicioPreferencias _preferencias = ServicioPreferencias();
  ...
  Future<void> enviarDevolucionAFirestore(Devolucion devolucion) async {
    final prestamoRef = _db.collection('prestamos').doc(devolucion.prestamoId);

    final activoRef = _db.collection('activos').doc(devolucion.activoId);

    final devolucionRef = _db.collection('devoluciones').doc(devolucion.id);

    final mantenimientoRef = _db
        .collection('mantenimientos')
        .doc('mant_${devolucion.id}');

    final historialRef = _db
        .collection('historial')
        .doc('hist_${devolucion.id}');
    ...
    await _db.runTransaction((transaction) async {
      ...
      final estadoActivo = (datosActivo['estado'] ?? '').toString();
      ...
      transaction.update(prestamoRef, {
        ...
        'fechaDevolucion': Timestamp.fromDate(devolucion.fechaDevolucion),
      });
      ...
      if (devolucion.tieneNovedad) {
        ...
        final nuevaCantidadDisponible = cantidadDisponible < cantidadTotal
  ...
  Future<ResultadoSincronizacion> sincronizarPendientes() async {
    ...
    if (registrosPendientes.isEmpty) {
      return ResultadoSincronizacion(sincronizadas: 0, pendientes: 0);
    }
    ...
    for (final registro in registrosPendientes) {
      ...
      try {
        ...
        await _database.dejarPendiente(registro.id, mensaje);
    ...
    if (sincronizadas > 0) {
      await _preferencias.guardarUltimaSincronizacion(DateTime.now());
    }
